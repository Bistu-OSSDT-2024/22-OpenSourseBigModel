...
/*public static void main(String[] args) throws Exception {
	ExecutorService executorService = Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors()); // 使用CPU核心数作为线程池大小
	List<Future<Void>> futures = new ArrayList<>();

	for (int run = 0; run < 2*N_RUNS_HALF + 1; ++run) {
		final int currentRun = run; // 因为Runnable是匿名的，所以需要final变量来在内部类中访问它
		Callable<Solver> task = () -> {
			Solver solver = makeAndSaveSolver(args); // 假设makeAndSaveSolver方法存在
			if (solver.labyrinth.smallEnoughToDisplay()) {
				displayLabyrinth(solver); // 假设displayLabyrinth方法存在
			}

			long startTime = System.currentTimeMillis();
			solver.solution = solver.solve(); // 假设solve方法存在，并且会返回求解结果
			long endTime = System.currentTimeMillis();

			// ... 其他逻辑，比如打印结果和检查解决方案

			return solver; // 返回Solver对象以便后续处理
		};

		futures.add(executorService.submit(task).toString()); // 提交任务到线程池
	}

	// 收集结果
	for (Future<Solver> future : futures) {
		Solver solver = future.get(); // 这会阻塞，直到任务完成
		// 在这里处理solver对象，比如打印结果、检查解决方案等
		// 注意：由于future.get()会阻塞，所以这里实际上还是串行的，但任务本身是在多线程中并发执行的

		// 假设runTimes是一个全局数组或列表，用于存储每次运行的时间
		// 注意：在多线程环境中，需要同步访问共享资源
		// ...

		// 检查解决方案是否正确（这里只是一个示例，可能需要进一步处理）
		// ...
	}
